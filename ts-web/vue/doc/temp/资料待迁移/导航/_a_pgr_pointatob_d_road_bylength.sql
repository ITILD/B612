CREATE OR REPLACE FUNCTION public._a_pgr_pointatob_d_road_bylength(start_point geometry, start_pid character varying, end_point geometry, end_pid character varying)
 RETURNS TABLE(pid_this character varying, name_this character varying, geom_this geometry, length_cost_this double precision, time_cost_this double precision, rout_type_this character varying)
 LANGUAGE plpgsql
AS $function$
declare
...
if start_code is null then
		...
		RETURN query  select  pid as pid_this,name as name_this , geom as geom_this,
			...
			rout_type as rout_type_this from (
				select  pid,name,
				--1 起始截断
--				ST_LineMerge(ST_LocateBetween(ST_AddMeasure(geom,0,1),start_locate_start_ratio,start_locate_end_ratio)) as geom ,
				...
				A.cost as length_cost,
				(case A.node WHEN d_road.source then d_road.time_cost ELSE  d_road.time_reverse_cost END) as time_cost,
