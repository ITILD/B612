CREATE OR REPLACE FUNCTION public._a_pgr_pointatob_d_road_bytime(start_point geometry, start_pid character varying, end_point geometry, end_pid character varying)
 RETURNS TABLE(pid_this character varying, name_this character varying, geom_this geometry, time_cost_this double precision, length_cost_this double precision, rout_type_this character varying)
 LANGUAGE plpgsql
AS $function$
declare
...
begin
	--最短路径sql_时间优先
	sql_dijkstra:='SELECT gid AS id,source, target,time_cost as cost, time_reverse_cost as reverse_cost  FROM d_road';
	--获取最短路径点
	--查询离起点最近的线上点
	select 
			gid
		...
		into 
			...
			,start_reverse_cost
	FROM d_road 
	WHERE ST_DWithin(geom, start_point,100) and pid = start_pid and geom notnull
	...
	into 
		...
		,end_reverse_cost
	FROM d_road 
	WHERE ST_DWithin(geom, end_point,100) and pid = end_pid and geom notnull 
...
if start_code is null then
		...
		if start_onroad_to_source_cost > 0 and start_reverse_cost*start_onroad_ratio-end_reverse_cost*end_onroad_ratio>0.000001 then
			...
			,(start_locate_start_ratio-start_locate_end_ratio)*time_cost as time_cost_this
			,(start_locate_start_ratio-start_locate_end_ratio)*cost as length_cost_this
			,rout_type as rout_type_this
			from d_road where gid = start_road_gid ;
		else
			RETURN query  select  pid as pid_this,name as name_this,ST_LineMerge(ST_LocateBetween(ST_AddMeasure(geom,1,0),start_locate_start_ratio,start_locate_end_ratio)) as geom_this 
			,(start_locate_start_ratio-start_locate_end_ratio)*time_reverse_cost as time_cost_this
			,(start_locate_start_ratio-start_locate_end_ratio)*cost as length_cost_this
			,rout_type as rout_type_this
			from d_road where gid = start_road_gid;
		end if;
		...
		RETURN query  select  pid as pid_this,name as name_this , geom as geom_this,
			...
			rout_type as rout_type_this from (
				--1 起始截断
				select  pid,name,
				(case start_code WHEN target 
					then ST_LineMerge(ST_LocateBetween(ST_AddMeasure(geom,0,1),start_locate_start_ratio,start_locate_end_ratio)) 
					ELSE st_reverse(ST_LineMerge(ST_LocateBetween(ST_AddMeasure(geom,0,1),start_locate_start_ratio,start_locate_end_ratio))) END) 
				as geom ,
				--
				(start_locate_end_ratio-start_locate_start_ratio)*time_reverse_cost as time_cost,
				(start_locate_end_ratio-start_locate_start_ratio)*cost as length_cost,
				rout_type from d_road where gid = start_road_gid
				-- 起始点一样不需要拼接
				and start_locate_start_ratio!=start_locate_end_ratio
				--2 中间段最短路径查询
					union all
				select  pid,name,
				-- 单段方向判定
				(case A.node WHEN d_road.source then geom ELSE st_reverse(geom) END) as geom,
				A.cost as time_cost,
				-- 根据路线正反
				(case A.node WHEN d_road.source then d_road.cost ELSE  d_road.reverse_cost END) as length_cost,
				rout_type from pgr_dijkstra(sql_dijkstra,start_code,end_code,directed := true ) as A LEFT JOIN d_road on  A.edge = d_road.gid 
				--3 终截断
					union all
				select  pid,name,
				-- 方向
				(case end_code WHEN source 
					then ST_LineMerge(ST_LocateBetween(ST_AddMeasure(geom,0,1),end_locate_start_ratio,end_locate_end_ratio)) 
					ELSE st_reverse(ST_LineMerge(ST_LocateBetween(ST_AddMeasure(geom,0,1),end_locate_start_ratio,end_locate_end_ratio))) END) 
				as geom ,
				(end_locate_end_ratio-end_locate_start_ratio)*time_reverse_cost as time_cost,
				(end_locate_end_ratio-end_locate_start_ratio)*cost as length_cost,
				rout_type from d_road where gid = end_road_gid 
				-- 起始点一样不需要拼接
				and end_locate_end_ratio!=end_locate_start_ratio
				
				
--				!!!!重合点TODO
		) as B where time_cost notnull and  length_cost notnull ;
	end if;
